...
exports.handler = async (event) => {
  ...
  if (body.resumeLookup) {
    return handleResumeLookup(body);
  }
  ...
  try {
    ...
    if (body.resume && !resumeTarget) {
      return jsonResponse(410, { error: 'This update link is invalid or has already been used. Please contact clientservices@bglafrica.com.' });
    }
    ...
    return jsonResponse(200, { applicationReference, applicationId: realApplicationId });
  } catch (err) {
...
async function handleResumeLookup(body) {
  ...
  try {
    ...
    return jsonResponse(500, { error: 'Something went wrong loading your application. Please try again.' });
  }
}
